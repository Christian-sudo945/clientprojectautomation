...
import os
from dispatcher.Objects.WebDriverFunctions import *
from dispatcher.variables.Elements import *
from dispatcher.auto_dispatch import auto_dispatch
...
def run_gui(browser_automation):
    ...
    root.title("SAP CUP Tool Automation")
    ...
    # Hint style
    custom_style.configure("Hint.TLabel", 
                 font=("Aptos", 9),
                 foreground=colors["secondary"])
    ...
    # Form fields - function to create each field
    def create_form_field(row, label_text):
        label = ttk.Label(
            form_frame,
            text=f"{label_text}:",
            font=("Aptos", 10),
        ...
        entry.grid(row=row, column=1, sticky="ew", padx=(10, 0), pady=12)
        
        return entry
    
    # Create form fields
    entry_link = create_form_field(0, "SAP CUP Tool URL")
    entry_dispatcher = create_form_field(1, "Dispatcher Username")
    entry_variant = create_form_field(2, "Variant Name")
    entry_approvers = create_form_field(3, "Approver Usernames")
    
    # Hint for multiple approvers
    hint_frame = ttk.Frame(form_frame)
    hint_frame.grid(row=4, column=1, sticky="ew", padx=(10, 0))
    
    hint_label = ttk.Label(
        hint_frame,
        text="Enter comma-separated values for multiple approvers",
        style="Hint.TLabel"
    )
    ...
    status_frame.pack(fill="x", expand=False, padx=5, pady=(5, 10))
    ...
    def on_button_click():
        ...
        dispatcher = entry_dispatcher.get().strip()
        variant = entry_variant.get().strip()
        approver_usernames_str = entry_approvers.get().strip()
        approver_usernames = [username.strip() for username in approver_usernames_str.split(',') if username.strip()]
        
        if not link:
            messagebox.showerror("Input Error", "SAP CUP Tool Link cannot be empty.")
            return
        if not dispatcher:
            messagebox.showerror("Input Error", "Dispatcher Username cannot be empty.")
            return
        if not variant:
            messagebox.showerror("Input Error", "Variant Name cannot be empty.")
            return
        if not approver_usernames:
            messagebox.showerror("Input Error", "Approver Usernames cannot be empty.")
            return
        ...
        try:
            auto_dispatch(browser_automation, link, approver_usernames, variant, dispatcher)
