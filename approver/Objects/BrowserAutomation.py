...
class EdgeAutomation:
    def __init__(self):
        ...
        self.approver_dir = os.path.dirname(self.script_dir)
        self.root_dir = os.path.dirname(self.approver_dir)
        ...
        self.driver = None
        self.script_logs = os.path.join(self.approver_dir, 'logs', 'script_logs.csv')
        self.approve_logs = os.path.join(self.approver_dir, 'logs', 'approve_logs.csv')
        self.webdriver_timeout = 20
        
        # Create logs directory if it doesn't exist
        os.makedirs(os.path.join(self.approver_dir, 'logs'), exist_ok=True)
    ...
    def ticket_log(self, ticket_num, file_path):
        """
        Appends a log message to the approve_log CSV file
        """
        file_exists = os.path.isfile(file_path)
        ...
        with open(file_path, mode='a', newline='') as file:
            ...
            if not file_exists:
                writer.writerow(['Timestamp', 'Ticket Number'])
            
            writer.writerow([datetime.now(), ticket_num])
    ...
    def read_link_from_file(self):
        try:
            link_file = os.path.join(self.approver_dir, "link.txt")
